...
def test_kan_89_upgrade_matches_fresh_bootstrap_schema():
    canonical = db_setup.OCR_RESULT_COLLECTOR_SCHEMA.read_text(encoding="utf-8")
    migration = (
        db_setup.SQL_DIR / "migrations" / "kan_89_ocr_result_collector.sql"
    ).read_text(encoding="utf-8")
    assert canonical[canonical.index("BEGIN;"):] == migration[migration.index("BEGIN;"):]
...
def _write_upgrade_files(tmp_path):
    paths = {
        ...
        "ocr_collector": tmp_path / "kan_89_ocr_result_collector.sql",
    ...
    paths["ocr_collector"].write_text("CREATE TABLE IF NOT EXISTS budget.ocr_result_events(message_id text);", encoding="utf-8")
...
def _patch_upgrade_files(monkeypatch, paths):
    ...
    monkeypatch.setattr(db_setup, "OCR_RESULT_COLLECTOR_SCHEMA", paths["ocr_collector"])
...
def test_existing_base_schema_applies_shared_upgrades_and_ensures_receipt_schema(tmp_path, monkeypatch):
    ...
    conn.execute.assert_any_call("CREATE TABLE IF NOT EXISTS budget.ocr_result_events(message_id text);")
...
def test_blank_database_loads_base_and_shared_schemas_before_receipt_schema(tmp_path, monkeypatch):
    ...
    conn.execute.assert_any_call("CREATE TABLE IF NOT EXISTS budget.ocr_result_events(message_id text);")
    conn.execute.assert_any_call("CREATE TABLE IF NOT EXISTS budget.receipt_reprocess_requests(request_id uuid);")
    assert conn.commit.call_count == 11
    ensure.assert_called_once_with(conn, paths["template"])
