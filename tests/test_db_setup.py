...
def test_existing_base_schema_only_ensures_receipt_schema(tmp_path, monkeypatch):
    conn = MagicMock()
...
def test_blank_database_loads_base_schema_before_receipt_schema(tmp_path, monkeypatch):
    conn = MagicMock()
    ...
    constraints.write_text("SELECT 1;", encoding="utf-8")
    template.write_text("template", encoding="utf-8")
    ...
    conn.execute.assert_any_call("SELECT 1;")
