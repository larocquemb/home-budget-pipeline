...
class MatchResult:
    ...
    score: float
    disposition: str  # matched | ambiguous | new
    reason: str
...
def candidate_score(receipt, candidate: dict) -> float:
    """Score whether a receipt evidence record represents an existing expense."""
    score = 0.0
...
def find_match(conn, receipt, schema: str = "budget") -> MatchResult:
    """Find an existing canonical expense without guessing through ambiguity."""
...
def upsert_evidence(conn, receipt, schema: str = "budget", evidence_type: str = "scanned", expense_pk: Optional[int] = None) -> int:
    ...
    payload = {
        ...
        "payer": getattr(receipt, "payer", None),
    }
    with conn.cursor() as cur:
        cur.execute(
            f"""
...
def attach_evidence(conn, evidence_id: int, expense_pk: int, schema: str = "budget") -> None:
    ...
    with conn.cursor() as cur:
        cur.execute(
            f"UPDATE {schema}.receipt_evidence SET expense_pk = %s, updated_at = NOW() WHERE id = %s",
            (expense_pk, evidence_id),
        )
