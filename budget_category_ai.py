#!/usr/bin/env python3
"""Shared AI category fallback engine for Costco and Instacart item classification."""
...
class AICategoryEngine:
    def __init__(self, suggestions_path: Path) -> None:
        ...
        self.stats = {
            ...
            'no_result': 0,
            'api_errors': 0,
            'call_seconds_total': 0.0,
            'call_seconds_max': 0.0,
            'call_items_max': 0,
            'slow_calls': 0,
            'duration_le_2s': 0,
            'duration_2_to_5s': 0,
    ...
    def save_suggestions(self) -> None:
        try:
            ...
            self.suggestions_path.write_text(
                json.dumps(persistent, ensure_ascii=True, indent=2, sort_keys=True),
                encoding='utf-8',
            )
        except Exception:
            return
    ...
    def try_category_batch(self, items: List[Dict[str, str]], source_label: str = 'item') -> Dict[str, Optional[str]]:
        ...
        for attempt in range(2):
            ...
            try:
                ...
                if self.consecutive_errors >= self.max_consecutive_errors:
                    ...
                    print(
                        'AI fallback disabled after repeated API errors '
    ...
    def try_single_category(self, desc: str, sku: Optional[str] = None, source_label: str = 'item') -> Optional[str]:
        cached = self.get_cached_category(desc)
        if cached:
            return cached
        results = self.try_category_batch([{'id': 'single', 'sku': sku or '', 'desc': desc}], source_label=source_label)
