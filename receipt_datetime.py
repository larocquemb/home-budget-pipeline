...
MDY_RE = re.compile(
    r"\b(\d{1,2})/(\d{1,2})/(20\d{2}|\d{2})(?:\s+(\d{1,2}):(\d{2})(?::(\d{2}))?)?\b"
)

# OCR terminal examples: DATE/TIME: 26/05,'31 10:35:11 or PIES TIME : 26/06/03 17:30:42
YMD2_RE = re.compile(
    r"\b(\d{2})/\s*(\d{1,2})\s*[,']?\s*/?\s*[,']?\s*(\d{1,2})(?:\s+(\d{1,2}):(\d{2})(?::(\d{2}))?)?\b"
)
...
def extract_transaction_datetime(text: str) -> Optional[str]:
    ...
    # Prefer explicit/ordinary North American MM/DD/YY before terminal YY/MM/DD.
    for line in text.splitlines():
    ...
    # Terminal YY/MM/DD is especially common after DATE/TIME, TIME, ACCT or approval blocks.
    for line in text.splitlines():
