...
def receipt_patterns(receipt: str) -> tuple[str, ...]:
    ...
    A numeric selector such as ``1`` maps to the scanned receipt convention
    ``receipt_0001.pdf``. A filename/path selector is matched by basename.
    """
    value = receipt.strip()
    if not value:
...
def resolve_item_ids(dsn: str, receipt: str, merchant: str = "") -> tuple[int, ...]:
    ...
    patterns = receipt_patterns(receipt)
    ...
    if merchant_filter:
        where = f"({where}) AND e.store_name ILIKE %s"
