...
CREATE TABLE budget.receipt_ocr_lines (
    id BIGSERIAL PRIMARY KEY,
    evidence_id BIGINT NOT NULL REFERENCES budget.receipt_evidence(id) ON DELETE CASCADE,
...
CREATE TABLE budget.receipt_ocr_runs (
...
CREATE TABLE budget.receipt_ocr_passes (
...
CREATE TABLE budget.receipt_ocr_feedback (
...
CREATE INDEX idx_receipt_ocr_lines_context
    ON budget.receipt_ocr_lines (evidence_id, department, line_type);
CREATE INDEX idx_receipt_ocr_runs_evidence
    ON budget.receipt_ocr_runs (evidence_id, processed_at DESC);
CREATE INDEX idx_receipt_ocr_passes_profile
...
CREATE VIEW budget.receipt_ocr_effectiveness AS
SELECT
