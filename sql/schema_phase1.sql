...
-- Time-bounded payment-card reference data. Cards can later be linked directly
-- to accounts without changing receipt evidence already captured by last four.
...
CREATE TRIGGER trg_category_groups_set_updated_at
BEFORE UPDATE ON budget.category_groups
...
CREATE TRIGGER trg_receipt_evidence_set_updated_at
BEFORE UPDATE ON budget.receipt_evidence
FOR EACH ROW EXECUTE FUNCTION budget.set_updated_at();

CREATE TRIGGER trg_receipt_annotations_set_updated_at
BEFORE UPDATE ON budget.receipt_annotations
FOR EACH ROW EXECUTE FUNCTION budget.set_updated_at();
...
JOIN budget.expense_categories ec
    ON ec.category_name = ei.budget_category
JOIN budget.category_groups cg
    ON cg.id = ec.group_id
...
LEFT JOIN budget.account_balances b
